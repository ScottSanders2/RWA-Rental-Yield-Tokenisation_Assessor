...
class TestPropertyRegistration:
    ...
    def test_register_property_invalid_deed_hash(self, test_client):
        ...
        # Check that deed_hash field has an error
        assert any("deed_hash" in str(err).lower() for err in detail)
    ...
    def test_register_property_invalid_rental_uri(self, test_client):
        ...
        detail = response.json()["detail"]
        assert isinstance(detail, list)
        # Check that rental_agreement_uri field has an error
        assert any("rental_agreement_uri" in str(err).lower() for err in detail)
    ...
    def test_register_property_blockchain_failure(self, test_client, mock_web3_service, property_factory):
        ...
        # For 500 errors, detail is a string
        detail = response.json()["detail"]
        assert isinstance(detail, str)
        assert "blockchain" in detail.lower() or "error" in detail.lower()
...
class TestPropertyVerification:
    ...
    def test_verify_property_success(self, test_client, mock_web3_service, test_db):
        """Test successful property verification."""
        # Create an UNVERIFIED property directly in database (bypassing registration)
        from models.property import Property
        from datetime import datetime
        
        property_obj = Property(
            property_address_hash=bytes.fromhex("1" * 64),
    ...
    def test_verify_property_not_found(self, test_client):
        ...
        # Check for either "not found" or "invalid request" (both are acceptable error messages)
...
class TestPropertyQueries:
    ...
    def test_get_property_details(self, test_client, mock_web3_service, property_factory):
        ...
        assert data["is_verified"] is True  # Property is auto-verified during registration
        assert data["blockchain_token_id"] > 0  # Dynamic assertion for persistent blockchain
        assert "owner_address" in data  # New required field
        assert "has_active_yield_agreement" in data  # New required field
        assert data["has_active_yield_agreement"] is False  # No agreements yet
        assert isinstance(data["blockchain_token_id"], int)
