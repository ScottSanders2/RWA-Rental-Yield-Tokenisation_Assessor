...
class Web3Service:
    ...
    def __init__(self, testing_mode: bool = None, db: Session = None):
        ...
        if not self.testing_mode:
            ...
            # Load KYCRegistry if configured
            kyc_registry_address = self.contract_addresses.get("KYCRegistry")
            if kyc_registry_address:
                self.kyc_registry: Contract = self._get_contract_instance(
                    kyc_registry_address, "KYCRegistry"
                )
            else:
                self.kyc_registry = None
                logger.warning("KYCRegistry address not configured")
        else:
            ...
            self._mock_token_counter = 1  # Counter for generating mock token IDs
    ...
    def _get_next_mock_token_id(self) -> int:
        """
        Get next mock token ID for testing mode.
        
        Returns:
            Mock token ID (increments counter)
        """
        if not self.testing_mode:
            raise RuntimeError("_get_next_mock_token_id should only be called in testing mode")
    ...
    def _load_contract_abi(self, contract_name: str) -> Dict[str, Any]:
        ...
        For Diamond proxy contracts (YieldBase, CombinedPropertyYieldToken),
        loads the combined ABI file that includes all facets.
        ...
        # Special handling for Diamond proxy contracts - use combined ABI
        if contract_name == "YieldBase":
            abi_path = artifacts_dir / "DiamondYieldBase_ABI.json"
        elif contract_name == "CombinedPropertyYieldToken":
            abi_path = artifacts_dir / "DiamondCombinedToken_ABI.json"
        else:
        ...
        try:
            with open(abi_path, 'r') as f:
                # Combined ABI files are arrays, regular artifacts are objects with "abi" key
                data = json.load(f)
                if isinstance(data, list):
                    return data  # Combined ABI file
                else:
                    return data["abi"]  # Regular Foundry artifact
    ...
    def verify_property_nft(self, token_id: int) -> Tuple[str, int]:
        ...
        """
        if self.testing_mode:
            # Return mock transaction in testing mode
            return "0xMockTxHash_verifyProperty", 50000
    ...
    def verify_property_combined(self, token_id: int) -> Tuple[str, int]:
        """
        Verify property by calling CombinedPropertyYieldToken.verifyProperty().

        Args:
            token_id: CombinedPropertyYieldToken property token ID
        ...
        tx_hash, gas_used, _ = self._send_transaction(
            self.combined_token.functions.verifyProperty,
            token_id
        )
        return tx_hash, gas_used
    ...
    def create_yield_agreement(
        ...
        Args:
            ...
            upfront_capital_usd: Initial capital amount in USD (scaled by 1e18)
        ...
        if self.testing_mode:
            ...
            mock_agreement_id = self._get_next_mock_token_id() + 1000
            mock_token_address = f"0x{'0' * 36}{mock_agreement_id:04X}"  # Mock ERC-20 token address
            return mock_agreement_id, mock_token_address, "0xMockTxHash_createYieldAgreement", 200000
    ...
    def mint_combined_property_token(self, property_address_hash: bytes, metadata_uri: str) -> Tuple[int, str, int]:
        ...
        if self.testing_mode:
            ...
            mock_token_id = self._get_next_mock_token_id()
            # Generate unique transaction hash to avoid database constraint violations
            mock_tx_hash = "0x" + hashlib.sha256(f"mintCombinedProperty_{mock_token_id}_{datetime.now()}".encode()).hexdigest()
            return mock_token_id, mock_tx_hash, 100000
    ...
    def mint_combined_yield_tokens(
        ...
        **kwargs
        ...
        ✅ ALIGNED WITH ERC-721: The contract validates msg.sender owns the property
        and mints yield tokens to msg.sender (the deployer wallet).

        Args:
            property_token_id: Property token ID
            capital: Initial capital amount (wei)
            capital_usd: Initial capital amount in USD (scaled by 1e18)
            term: Agreement term in months
            roi: Annual ROI in basis points
            grace_period_days: Grace period in days before default
            default_penalty_rate: Default penalty rate percentage
            allow_partial_repayments: Whether partial repayments are allowed
            allow_early_repayment: Whether early repayment is allowed
        ...
        if self.testing_mode:
            # Return mock data in testing mode
            mock_yield_token_id = self._get_next_mock_token_id() + 1000000  # Offset for yield tokens
            return (
                mock_yield_token_id,
                "0x9fE46736679d2D9a65F0992F2272dE9f3c7fa6e0",  # Mock CombinedPropertyYieldToken address
                "0xMockTxHash_mintCombinedYield",
                150000
            )
        
        # ✅ ALIGNED WITH ERC-721: No propertyOwner parameter needed
        # Contract validates msg.sender owns property and mints to msg.sender
        # mintYieldTokens(uint256,uint256,uint256,uint16,uint16,uint16,uint16,bool,bool)
        tx_hash, gas_used, receipt = self._send_transaction(
    ...
    async def create_governance_proposal(
        ...
        if self.testing_mode:
            ...
            return await self._create_governance_proposal_production(agreement_id, proposal_type, target_value, description)
    ...
    async def get_voting_power(
        ...
        Args:
            ...
            db: Database session (required for both testing and production modes in development)
        ...
        if self.testing_mode:
            ...
            return await self._get_voting_power_production(voter_address, agreement_id, token_standard, db)
    ...
    def _get_voting_power_testing(
        ...
        Returns:
            Voting power in WEI (not tokens) - frontend will convert to tokens for display.
            This ensures consistency with vote casting which uses Wei values.
        """
        ...
        if not db:
            ...
            return 0  # No voting power without DB
        
        try:
            # Query actual token balance from user_share_balances (same as vote validation)
            user_balance = db.query(UserShareBalance).filter(
                UserShareBalance.user_address == voter_address.lower(),  # Normalize to lowercase
                UserShareBalance.agreement_id == agreement_id
            ).first()
            
            if user_balance and user_balance.balance_wei > 0:
                # Return Wei value (frontend will convert to tokens for display)
                voting_power_wei = int(user_balance.balance_wei)
                voting_power_tokens = voting_power_wei // 10**18  # For logging only
                logger.info(f"✅ Retrieved voting power for {voter_address[:10]}... on agreement #{agreement_id}: {voting_power_tokens:,} tokens ({voting_power_wei} Wei)")
                return voting_power_wei  # Return Wei, not tokens
            else:
            ...
            # Fallback to 0 - user has no voting power if query fails
            return 0
    ...
    async def _get_voting_power_production(
        ...
        In development (Anvil), we query the database since shares are minted on-chain
        but the deployer transfers them to property owners in the database.
        This creates a temporary mismatch where on-chain balances don't reflect
        logical ownership until marketplace transfers occur.
        
        Args:
            voter_address: Address to check voting power for
            agreement_id: Agreement ID
            token_standard: Token standard (ERC721 or ERC1155)
            db: Database session for querying user share balances
        ...
        # Use database as source of truth for voting power (same as testing mode)
        # This is correct for development since we initialize user_share_balances
        # when agreements are created, reflecting logical ownership
        try:
            # Query actual token balance from user_share_balances
            user_balance = db.query(UserShareBalance).filter(
                UserShareBalance.user_address == voter_address.lower(),  # Normalize to lowercase
                UserShareBalance.agreement_id == agreement_id
            ).first()
    ...
    def is_transfer_allowed(
        ...
        Args:
            ...
            amount: Amount to transfer (in wei/base units)
            agreement_id: Optional agreement ID for ERC-1155 lookup
            token_standard: "ERC20" or "ERC1155"
        
        Returns:
            Tuple of (allowed, reason) where reason is empty string if allowed
        """
        if self.testing_mode:
            return self._is_transfer_allowed_testing(from_address, to_address, amount)
        else:
            return self._is_transfer_allowed_production(
                token_contract_address, from_address, to_address, amount, agreement_id, token_standard
            )
    
    def _is_transfer_allowed_testing(self, from_address: str, to_address: str, amount: int) -> Tuple[bool, str]:
        """Testing mode implementation - always allow transfers"""
        logger.info(f"🧪 Testing mode: Transfer check passed for {amount} tokens from {from_address} to {to_address}")
        return (True, "")
    
    def _is_transfer_allowed_production(
        self,
        token_contract_address: str,
        from_address: str,
        to_address: str,
        ...
        """Production mode implementation - check on-chain restrictions"""
        try:
            if token_standard == "ERC1155":
                # For ERC-1155, need yield token ID
                if not agreement_id:
                    logger.warning("Agreement ID required for ERC-1155 transfer check")
                    return (False, "Agreement ID required for ERC-1155")
                
                # Get yield token ID from governance controller
                governance_abi = self._load_contract_abi("GovernanceController")
                governance_address = self.contract_addresses.get("GovernanceController")
                
                if not governance_address:
                    logger.warning("GovernanceController address not found")
                    return (True, "")  # Allow if governance not configured
                ...
                if yield_token_id == 0:
                    logger.warning(f"No yield token ID mapping for agreement {agreement_id}")
                    return (True, "")  # Allow if mapping not found
                
                # Call isYieldTokenTransferAllowed on CombinedPropertyYieldToken
                allowed, reason = self.combined_token.functions.isYieldTokenTransferAllowed(
                    yield_token_id,
                    from_address,
                    to_address,
                    amount
                ...
                # For ERC-20, get token contract and call isTransferAllowed
                token_abi = self._load_contract_abi("YieldSharesToken")
                token_contract = self.w3.eth.contract(address=token_contract_address, abi=token_abi)
                
                allowed, reason = token_contract.functions.isTransferAllowed(
                    from_address,
                    to_address,
                    amount
                ).call()
            ...
            logger.error(f"Error checking transfer restrictions: {e}")
            # On error, allow transfer to prevent blocking legitimate trades
            return (True, "")
    ...
    def execute_transfer(
        ...
        """
        if self.testing_mode:
            return self._execute_transfer_testing(from_address, to_address, amount)
        else:
            return self._execute_transfer_production(
                token_contract_address, from_address, to_address, amount, agreement_id, token_standard
            )
    
    def _execute_transfer_testing(self, from_address: str, to_address: str, amount: int) -> Tuple[str, int]:
        """Testing mode implementation - generate mock transaction"""
        import hashlib
        import time
        
        unique_string = f"{time.time()}-{from_address}-{to_address}-{amount}"
        tx_hash = "0x" + hashlib.sha256(unique_string.encode()).hexdigest()
        gas_used = 85000  # Realistic gas estimate for token transfer
        
        logger.info(f"🧪 Testing mode: Mock transfer of {amount} tokens from {from_address} to {to_address}")
        logger.info(f"   TX Hash: {tx_hash}, Gas: {gas_used}")
        
        return (tx_hash, gas_used)
    
    def _execute_transfer_production(
        self,
        token_contract_address: str,
        from_address: str,
        to_address: str,
        ...
        """Production mode implementation - execute on-chain transfer"""
        try:
            if token_standard == "ERC1155":
                # For ERC-1155, use safeTransferFrom with yield token ID
                if not agreement_id:
                    raise ValueError("Agreement ID required for ERC-1155 transfer")
                ...
                if not governance_address:
                    raise ValueError("GovernanceController address not found")
                
                governance_contract = self.w3.eth.contract(address=governance_address, abi=governance_abi)
                yield_token_id = governance_contract.functions.agreementToYieldTokenId(agreement_id).call()
                
                if yield_token_id == 0:
                    raise ValueError(f"No yield token ID mapping for agreement {agreement_id}")
                
                # Execute safeTransferFrom
                tx_hash, gas_used, receipt = self._send_transaction(
                    self.combined_token.functions.safeTransferFrom,
                    from_address,
                    to_address,
                    yield_token_id,
                    amount,
                ...
                # For ERC-20, use transferFrom
                token_abi = self._load_contract_abi("YieldSharesToken")
                token_contract = self.w3.eth.contract(address=token_contract_address, abi=token_abi)
                
                # Execute transferFrom (requires seller to have approved marketplace/escrow)
                tx_hash, gas_used, receipt = self._send_transaction(
                    token_contract.functions.transferFrom,
                    from_address,
                    to_address,
                    amount
                )
    ...
    # ============================================================================
    # SHARE TRANSFER FUNCTIONS (for ownership alignment)
    # ============================================================================
    
    def transfer_erc20_shares(
        self,
        token_contract_address: str,
        to_address: str,
        amount: int
        ...
        """
        Transfer ERC-20 YieldSharesToken from deployer to recipient.
        
        Used to align on-chain ownership with database records after minting.
        The deployer owns shares initially, then transfers them to the logical owner.
        
        Args:
            token_contract_address: Address of YieldSharesToken contract
            to_address: Recipient address (logical owner from database)
            amount: Number of shares to transfer (in wei)
            
        Returns:
            Tuple of (transaction_hash, gas_used)
            
        Raises:
            ValueError: If contract address or recipient invalid
            Exception: If transfer fails
        """
        if self.testing_mode:
            # Testing mode - simulate successful transfer
            logger.info(f"[TEST MODE] Simulated ERC-20 transfer: {amount} shares to {to_address}")
            return ("0x" + "0" * 64, 21000)
        
        try:
            # Convert amount to int (in case it's Decimal from database)
            amount = int(amount)
            ...
            # Load YieldSharesToken ABI and create contract instance
            token_abi = self._load_contract_abi("YieldSharesToken")
            token_contract = self.w3.eth.contract(address=token_address_checksum, abi=token_abi)
            
            # Execute transfer (deployer sends from their own balance)
            tx_hash, gas_used, receipt = self._send_transaction(
            ...
            logger.info(f"✅ Transferred {amount / 10**18} ERC-20 shares to {to_address[:10]}... (tx: {tx_hash[:10]}...)")
            
            return (tx_hash, gas_used)
            
        except Exception as e:
            logger.error(f"❌ ERC-20 transfer failed: {e}")
            raise
    
    def safe_transfer_erc1155_shares(
        self,
        to_address: str,
        yield_token_id: int,
        amount: int
        ...
        """
        Transfer ERC-1155 yield tokens from deployer to recipient.
        
        Used to align on-chain ownership with database records after minting.
        The deployer owns yield tokens initially, then transfers them to the logical owner.
        
        Args:
            to_address: Recipient address (logical owner from database)
            yield_token_id: ERC-1155 token ID for the yield tokens
            amount: Number of yield tokens to transfer (in wei)
            
        Returns:
            Tuple of (transaction_hash, gas_used)
            
        Raises:
            ValueError: If recipient address invalid
            Exception: If transfer fails
        """
        if self.testing_mode:
            # Testing mode - simulate successful transfer
            logger.info(f"[TEST MODE] Simulated ERC-1155 transfer: {amount} tokens (ID: {yield_token_id}) to {to_address}")
            return ("0x" + "0" * 64, 21000)
        
        try:
            # Convert values to int (in case they're Decimal from database)
            yield_token_id = int(yield_token_id)
            amount = int(amount)
            
            # Validate addresses
            to_address_checksum = Web3.to_checksum_address(to_address)
            from_address_checksum = Web3.to_checksum_address(self.deployer_account.address)
            
            # Execute safeTransferFrom (deployer → recipient)
            tx_hash, gas_used, receipt = self._send_transaction(
                self.combined_token.functions.safeTransferFrom,
                from_address_checksum,  # from (deployer)
                to_address_checksum,    # to (logical owner)
                yield_token_id,         # token ID
                amount,                 # amount
                b""                     # data (empty)
            )
            
            logger.info(f"✅ Transferred {amount / 10**18} ERC-1155 tokens (ID: {yield_token_id}) to {to_address[:10]}... (tx: {tx_hash[:10]}...)")
            
            return (tx_hash, gas_used)
            
        except Exception as e:
            logger.error(f"❌ ERC-1155 transfer failed: {e}")
            raise
    
    # ============================================================================
    # KYC Whitelist Management Methods
    # ============================================================================
    
    def add_to_kyc_whitelist(self, address: str) -> str:
        """
        Add address to KYC whitelist on-chain
        
        Args:
            address: Ethereum address to whitelist
        
        Returns:
            str: Transaction hash
        
        Raises:
            ValueError: If KYC Registry not configured
            Exception: If transaction fails
        """
        if self.testing_mode:
            logger.info(f"[TESTING MODE] Would add {address} to KYC whitelist")
            return "0x" + "0" * 64
        
        if not self.kyc_registry:
            raise ValueError("KYC Registry not configured")
        ...
        try:
            ...
            tx_hash, gas_used, receipt = self._send_transaction(
                self.kyc_registry.functions.addToWhitelist,
                address_checksum
            )
            
            logger.info(f"✅ Added {address} to KYC whitelist (tx: {tx_hash[:10]}..., gas: {gas_used})")
            
            # Log to audit trail
            self.transaction_history.append({
                'type': 'kyc_whitelist_add',
                'address': address,
                'tx_hash': tx_hash,
            ...
            logger.error(f"❌ Failed to add {address} to KYC whitelist: {e}")
            raise
    
    def remove_from_kyc_whitelist(self, address: str) -> str:
        """
        Remove address from KYC whitelist on-chain
        
        Args:
            address: Ethereum address to remove from whitelist
        ...
        """
        if self.testing_mode:
            logger.info(f"[TESTING MODE] Would remove {address} from KYC whitelist")
            return "0x" + "0" * 64
        ...
        try:
            ...
            tx_hash, gas_used, receipt = self._send_transaction(
                self.kyc_registry.functions.removeFromWhitelist,
                address_checksum
            )
            
            logger.info(f"✅ Removed {address} from KYC whitelist (tx: {tx_hash[:10]}..., gas: {gas_used})")
            
            # Log to audit trail
            self.transaction_history.append({
                'type': 'kyc_whitelist_remove',
                'address': address,
                'tx_hash': tx_hash,
                'timestamp': self.w3.eth.get_block('latest')['timestamp']
            })
            ...
            logger.error(f"❌ Failed to remove {address} from KYC whitelist: {e}")
            raise
    
    def batch_add_to_whitelist(self, addresses: List[str]) -> str:
        """
        Batch add addresses to whitelist (gas efficient)
        
        Args:
            addresses: List of Ethereum addresses to whitelist
        
        Returns:
            str: Transaction hash
        
        Raises:
            ValueError: If KYC Registry not configured or too many addresses
            Exception: If transaction fails
        """
        if self.testing_mode:
            logger.info(f"[TESTING MODE] Would batch add {len(addresses)} addresses to KYC whitelist")
            return "0x" + "0" * 64
        ...
        if len(addresses) > 100:
            raise ValueError("Too many addresses (max 100 per batch)")
        
        try:
            # Convert addresses to checksum format
            addresses_checksum = [Web3.to_checksum_address(addr) for addr in addresses]
            
            tx_hash, gas_used, receipt = self._send_transaction(
                self.kyc_registry.functions.batchAddToWhitelist,
                addresses_checksum
            )
            
            logger.info(f"✅ Batch added {len(addresses)} addresses to KYC whitelist (tx: {tx_hash[:10]}..., gas: {gas_used})")
            
            return tx_hash
            
        except Exception as e:
            logger.error(f"❌ Failed to batch add addresses to KYC whitelist: {e}")
            raise
    
    def is_kyc_whitelisted(self, address: str) -> bool:
        """
        Check if address is KYC whitelisted on-chain
        ...
        """
        if self.testing_mode:
            return True  # All addresses whitelisted in testing mode
        
        if not self.kyc_registry:
            logger.warning("KYC Registry not configured, returning False")
            return False
        
        try:
            address_checksum = Web3.to_checksum_address(address)
            return self.kyc_registry.functions.isWhitelisted(address_checksum).call()
        except Exception as e:
            logger.error(f"Failed to check KYC whitelist status for {address}: {e}")
            return False
    
    def verify_signature(self, address: str, message: str, signature: str) -> bool:
        """
        Verify signed message for wallet ownership proof
        
        Args:
            address: Ethereum address that should have signed the message
            message: Original message that was signed
            signature: Signature hex string (0x-prefixed)
        
        Returns:
            bool: True if signature is valid, False otherwise
        """
        # Allow mock signatures for testing (even when testing_mode is false)
        # This enables end-to-end testing with mock test users while using real blockchain
        if signature.startswith("0xMockSignature_"):
            logger.info(f"✅ Mock signature detected and allowed for testing: {address}")
            return True
        
        if self.testing_mode:
            return True  # All signatures valid in testing mode
