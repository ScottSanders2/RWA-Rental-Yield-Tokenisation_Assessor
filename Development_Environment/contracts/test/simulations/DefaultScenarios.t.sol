...
contract DefaultScenariosTest is KYCTestHelper {
    ...
    function setUp() public override {
        ...
        whitelistAddress(propertyOwner);
        whitelistAddress(investor);
        ...
        linkKYCToYieldBase(address(yieldBase));
    ...
    function testSingleMissedPaymentWithinGracePeriod() public {
        ...
        // Fast forward 35 days (past the 30-day repayment period)
    ...
    function testMultipleMissedPaymentsReachingDefaultThreshold() public {
        ...
        // Force default with 4th missed payment to ensure threshold is reached
        vm.warp(block.timestamp + 35 days);
        vm.prank(owner);
        yieldBase.handleMissedPayment(agreementId);

        uint256 finalPenalty = YieldCalculations.calculateDefaultPenalty(monthlyPayment, 200, 4);
        totalExpectedPenalty += finalPenalty;

        (, bool isInDefault, uint8 missedCount, uint256 arrears, , , , ) = yieldBase.getAgreementStatus(agreementId);

        // Fourth missed payment should trigger default
        assertTrue(isInDefault, "Should be in default at threshold");
        assertEq(missedCount, 4, "Missed payment count should be 4");
        ...
        metrics.push(DefaultMetrics({
            ...
            gasCost: 0 // Gas cost not measured for final call
        }));
    }
    ...
    function testDefaultPreventsNewRepayments() public {
        ...
        for (uint8 i = 0; i < 3; i++) {
            ...
            // Verify state after each missed payment
            (, bool isInDefaultAfter, uint8 missedCount, , , , , ) = yieldBase.getAgreementStatus(agreementId);
            assertFalse(isInDefaultAfter, "Should not be in default during grace period");
            assertEq(missedCount, uint8(i + 1), "Missed payment count should be correct");
        }

        // Wait for grace period to expire (30 days after grace period started)
        vm.warp(block.timestamp + 31 days);

        // Check and update default status (this is what would happen in production)
        yieldBase.checkAndUpdateDefaultStatus(agreementId);

        // Verify defaulted
        (, bool isInDefault, uint8 finalMissedCount, , , , , ) = yieldBase.getAgreementStatus(agreementId);
        assertTrue(isInDefault, "Agreement should be defaulted after grace period expires");
        assertEq(finalMissedCount, 3, "Missed payment count should remain 3");

        // Attempt repayment (should revert due to default)
        vm.prank(propertyOwner);
        try yieldBase.makeRepayment{value: 0.1 ether}(agreementId) {
            // If it succeeds, that's wrong - agreement should be in default
            assertTrue(false, "makeRepayment should have failed for defaulted agreement");
        } catch {
            // Expected - repayment correctly blocked for defaulted agreement
        }
    }
