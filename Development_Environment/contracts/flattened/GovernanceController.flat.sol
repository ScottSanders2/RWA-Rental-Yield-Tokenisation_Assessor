...
pragma solidity <0.9.0 >=0.4.11 >=0.4.16 >=0.4.22 >=0.5.0 >=0.6.2 >=0.8.4 ^0.8.20 ^0.8.21 ^0.8.22 ^0.8.24;

// src/storage/CombinedTokenStorage.sol
...
library CombinedTokenStorage {
    ...
    struct CombinedTokenStorageLayout {
        ...
        mapping(uint256 => bool) yieldTokenRestrictionsEnabled; // yieldTokenId => restrictions enabled flag
        mapping(address => mapping(uint256 => uint256)) lastTransferTimestamp; // holder => tokenId => last transfer time
        address kycRegistry;                              // address of KYCRegistry contract for compliance checks
        address yieldBaseContract;                        // address of YieldBase Diamond for cross-contract calls
    }
...
// src/libraries/GovernanceCalculations.sol
...
// src/storage/GovernanceStorage.sol
...
library GovernanceStorage {
    ...
    enum ProposalType {
        ...
        KYCWhitelistUpdate           // Add or remove addresses from KYC whitelist via democratic governance
    }
...
// src/interfaces/IGovernanceController.sol
...
// src/storage/PropertyStorage.sol
...
// src/libraries/CombinedTokenDistribution.sol
...
// src/CombinedPropertyYieldToken.sol
...
/// 
/// @custom:deprecated ⚠️ DEPRECATED: This monolithic contract has been superseded by the Diamond Pattern (EIP-2535).
/// @custom:deprecated Use DiamondCombinedToken with MintingFacet, DistributionFacet, RestrictionsFacet, etc. instead.
/// @custom:deprecated Deploy via script/DeployDiamond.s.sol for modular, upgradeable architecture.
/// @custom:deprecated This contract is maintained for reference and legacy tests only - it is NOT deployed.
    ...
    ReentrancyGuardUpgradeable
{
    ...
    /// @param tokenAmount The amount of yield tokens minted
    /// @param upfrontCapital The upfront capital amount in Wei
    /// @param upfrontCapitalUsd The upfront capital amount in USD (scaled by 1e18)
    /// @param termMonths The repayment term in months
    /// @param annualROIBasisPoints The annual ROI in basis points
    /// @param gracePeriodDays Grace period before default penalties apply
    ...
    function mintPropertyToken(
        ...
        // KYC verification for regulatory compliance
        require(address(kycRegistry) != address(0), "KYC Registry not set");
        require(kycRegistry.isWhitelisted(msg.sender), "Owner not KYC verified");
        require(!kycRegistry.isBlacklisted(msg.sender), "Owner is blacklisted");
    ...
    function mintYieldTokens(
        ...
        // KYC verification for regulatory compliance
        require(address(kycRegistry) != address(0), "KYC Registry not set");
        require(kycRegistry.isWhitelisted(msg.sender), "Investor not KYC verified");
        require(!kycRegistry.isBlacklisted(msg.sender), "Investor is blacklisted");
    ...
    ReentrancyGuardUpgradeable
{
    ...
    /// @notice Emitted when the token counter is manually set (after redeployment)
    /// @param nextTokenId The new token counter value
    event TokenCounterUpdated(uint256 nextTokenId);
    ...
    function mintProperty(
        ...
        // Mint to msg.sender (owner/backend) for now
        // TODO: Add 'address to' parameter to mint to property owner
    ...
    /// @notice Set the token ID counter after contract redeployment
    /// @dev Emergency function to prevent token ID collisions with existing database records
    ///      Should be called immediately after deployment if migrating from previous deployment
    /// @param nextTokenId Next token ID to use (highest existing ID + 1)
    function setTokenCounter(uint256 nextTokenId) external onlyOwner {
        PropertyStorage.PropertyStorageLayout storage layout = PropertyStorage.getPropertyStorage();
        layout.nextTokenId = nextTokenId;
        
        emit TokenCounterUpdated(nextTokenId);
    }
    
    /// @notice Get the current token ID counter
    /// @return counter Current token counter value
...
/// 
/// @custom:deprecated ⚠️ DEPRECATED: This monolithic contract has been superseded by the Diamond Pattern (EIP-2535).
/// @custom:deprecated Use DiamondYieldBase with YieldBaseFacet, RepaymentFacet, GovernanceFacet, etc. instead.
/// @custom:deprecated Deploy via script/DeployDiamond.s.sol for modular, upgradeable architecture.
/// @custom:deprecated This contract is maintained for reference and legacy tests only - it is NOT deployed.
contract YieldBase is Initializable, UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    /// @notice Modifier that enforces KYC verification for regulatory compliance
    /// @dev Checks both whitelist status (must be true) and blacklist status (must be false)
    /// @param account The address to verify KYC status for
    modifier onlyKYCVerified(address account) {
        require(address(kycRegistry) != address(0), "KYC Registry not set");
        require(kycRegistry.isWhitelisted(account), "Address not KYC verified");
        require(!kycRegistry.isBlacklisted(account), "Address is blacklisted");
        _;
    }
    ...
    function createYieldAgreement(
        ...
        bool allowPartialRepayments,
        bool allowEarlyRepayment
    ) external nonReentrant onlyKYCVerified(msg.sender) returns (uint256 agreementId) {
    ...
    function makeRepayment(uint256 agreementId) external onlyOwnerOrPayer(agreementId) onlyKYCVerified(msg.sender) nonReentrant payable {
        YieldStorage.YieldData storage yieldData = _agreements[agreementId];
        require(yieldData.isActive, "Yield agreement is not active");
        require(yieldData.upfrontCapital > 0, "Agreement does not exist");
