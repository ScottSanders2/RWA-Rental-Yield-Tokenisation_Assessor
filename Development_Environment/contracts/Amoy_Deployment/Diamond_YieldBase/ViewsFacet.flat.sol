...
contract ViewsFacet is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    /// @notice Get the YieldSharesToken address for a given agreement
    /// @param agreementId The ID of the yield agreement
    /// @return tokenAddress The address of the YieldSharesToken contract for this agreement
    function getAgreementToken(uint256 agreementId) external view returns (address tokenAddress) {
        DiamondYieldStorage.AgreementStorage storage agreements = DiamondYieldStorage.getAgreementStorage();
        tokenAddress = address(agreements.agreementTokens[agreementId]);
    }
