...
# Extract .abi from each JSON, combine, and remove duplicates by function/event signature
# For functions/events: keep first occurrence based on name + input types
# This handles cases where functions have same name but different output names
jq -s '
    [.[] | .abi] | 
    add | 
    group_by(.type + (.name // "") + ((.inputs // []) | map(.type) | join(","))) |
    map(.[0])
