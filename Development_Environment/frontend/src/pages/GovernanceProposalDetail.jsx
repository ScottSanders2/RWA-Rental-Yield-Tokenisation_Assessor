...
function GovernanceProposalDetail() {
  ...
  const fetchVotingPower = async () => {
    ...
    try {
      ...
      setVotingPower(0); // Fallback to 0 tokens (user has no voting power if query fails)
    }
  };
  ...
  // Convert Wei (18 decimals) to human-readable token count
  const formatVotesFromWei = (weiValue) => {
    if (!weiValue || weiValue === 0) return '0';
    // Convert from Wei to tokens (divide by 10^18)
    const tokens = weiValue / 1e18;
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 4 }}>
            ...
            <CardContent>
              ...
              <Box sx={{ mb: 3 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                  ...
                  <Typography variant="body2" fontWeight="medium">
                    {formatVotesFromWei(proposal.for_votes)} votes ({votePercentages.forPercent.toFixed(1)}%)
                  </Typography>
                </Box>
                <LinearProgress 
                  variant="determinate" 
              ...
              <Box sx={{ mb: 3 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                  ...
                  <Typography variant="body2" fontWeight="medium">
                    {formatVotesFromWei(proposal.against_votes)} votes ({votePercentages.againstPercent.toFixed(1)}%)
                  </Typography>
                </Box>
                <LinearProgress 
                  variant="determinate" 
              ...
              <Box sx={{ mb: 2 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                  ...
                  <Typography variant="body2" fontWeight="medium">
                    {formatVotesFromWei(proposal.abstain_votes)} votes ({votePercentages.abstainPercent.toFixed(1)}%)
                  </Typography>
                </Box>
                <LinearProgress 
                  variant="determinate" 
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <Typography variant="body1" fontWeight="bold">
                  {formatVotesFromWei(proposal.for_votes + proposal.against_votes + proposal.abstain_votes)}
                </Typography>
              </Box>
            </CardContent>
          </Card>
      ...
      {proposal.status === 'ACTIVE' && (
        <Alert severity="info" sx={{ mt: 3 }}>
          <Typography variant="body2">
            <strong>Your Voting Power:</strong> {formatVotesFromWei(votingPower)} tokens
          </Typography>
          {hasVoted && (
      ...
      <Dialog open={voteDialogOpen} onClose={() => !voting && setVoteDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          ...
          <Typography variant="body2" color="text.secondary" paragraph>
            Your voting power: <strong>{formatVotesFromWei(votingPower)}</strong> tokens
          </Typography>
          <Typography variant="body2" color="text.secondary" paragraph>
