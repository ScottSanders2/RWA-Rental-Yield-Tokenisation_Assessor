...
describe('Formatter Utilities', () => {
  ...
  describe('formatWeiToUsd', () => {
    ...
    it('handles different price points', () => {
      // Same amount, different price (BUT function uses STATIC $2000 rate)
      // Price parameter is DEPRECATED - function always uses STATIC_ETH_USD_RATE = 2000
      expect(formatWeiToUsd('1000000000000000000', 2500)).toBe('$2,000.00');
      expect(formatWeiToUsd('1000000000000000000', 1500)).toBe('$2,000.00');
    });

    it('handles invalid inputs', () => {
      expect(formatWeiToUsd(null, 2000)).toBe('$0.00');
      // Price parameter is DEPRECATED - null price still uses STATIC_ETH_USD_RATE = 2000
      expect(formatWeiToUsd('1000000000000000000', null)).toBe('$2,000.00');
    });
  });
  ...
  describe('formatUsdToWei', () => {
    ...
    it('handles different price points', () => {
      // Same USD amount, different ETH price (BUT function uses STATIC $2000 rate)
      // Price parameter is DEPRECATED - function always uses STATIC_ETH_USD_RATE = 2000
      expect(formatUsdToWei('2000', 2500)).toBe('1000000000000000000'); // 1 ETH at $2000/ETH
      expect(formatUsdToWei('2000', 1500)).toBe('1000000000000000000'); // 1 ETH at $2000/ETH
    });

    it('handles invalid inputs', () => {
      expect(formatUsdToWei(null, 2000)).toBe('0');
      // Price parameter is DEPRECATED - null price still uses STATIC_ETH_USD_RATE = 2000
      expect(formatUsdToWei('1000', null)).toBe('500000000000000000'); // $1000 / $2000 = 0.5 ETH
    });
  });
  ...
  describe('validateEthereumAddress', () => {
    ...
    it('rejects invalid Ethereum addresses', () => {
      ...
      expect(validateEthereumAddress('1234567890123456789012345678901234567890')).toBe(false); // missing 0x prefix (our implementation requires it)
