...
const getBaseURL = () => {
  ...
  return API_BASE_URL || 'http://192.168.1.144:8000';
};
...
export const createListing = async (listingData) => {
  console.log('🏪 [MARKETPLACE API] Creating listing...');
  console.log('📦 Request payload:', JSON.stringify(listingData, null, 2));
  console.log('🌐 Target: POST /marketplace/listings');
  console.log('🔌 Base URL:', getBaseURL());
  
  const response = await apiClient.post('/marketplace/listings', listingData);
  
  console.log('🏪 [MARKETPLACE API] Listing created successfully');
