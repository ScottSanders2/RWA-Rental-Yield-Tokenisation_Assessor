...
contract MintingFacet is ERC1155Upgradeable, OwnableUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function mintYieldTokens(
    ...
    ) external nonReentrant returns (uint256 yieldTokenId) {
        // Validate inputs
        ...
        // Authorization: Platform minter (backend) OR property token owner can mint
        // This allows the platform to operate yield tokenization on behalf of property owners
        // after completing off-chain verification, KYC/AML, and legal compliance checks.
        bool isPlatformMinter = hasRole(PLATFORM_MINTER_ROLE, msg.sender);
        bool isPropertyOwner = balanceOf(msg.sender, propertyTokenId) == 1;
        
        if (!isPlatformMinter && !isPropertyOwner) {
        ...
        // CRITICAL FIX: Mint to property owner, NOT msg.sender (platform minter)
        // The property owner address is passed as a parameter by the backend
        // This ensures blockchain ownership matches database ownership
        require(propertyOwner != address(0), "Invalid property owner address");
        
        // Verify the provided property owner actually owns the property token
        // Only required if msg.sender is platform minter (backend)
        if (hasRole(PLATFORM_MINTER_ROLE, msg.sender)) {
            require(balanceOf(propertyOwner, propertyTokenId) == 1, "Property owner must own property token");
        }
        
        _mint(propertyOwner, yieldTokenId, tokenAmount, ""); // Mint fungible yield tokens to property owner
