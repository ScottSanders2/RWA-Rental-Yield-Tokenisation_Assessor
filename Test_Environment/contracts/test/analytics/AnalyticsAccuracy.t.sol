...
contract AnalyticsAccuracyTest is Test {
    ...
    // Graph Node endpoint: use localhost:8200 for host tests, or rwa-dev-graph-node:8000 for container tests
    // Can be configured via GRAPH_NODE_URL environment variable
    string GRAPH_NODE_ENDPOINT = "http://localhost:8200/subgraphs/name/rwa-tokenization";

    function setUp() public {
        // Check for GRAPH_NODE_URL environment variable
        try vm.envString("GRAPH_NODE_URL") returns (string memory envUrl) {
            GRAPH_NODE_ENDPOINT = envUrl;
        } catch {
        ...
        governanceController = GovernanceController(payable(address(governanceController)));
        ...
        // Create a test agreement to initialize yieldSharesToken
        ...
        // Get YieldSharesToken address from the created agreement
        address tokenAddress = yieldBase.getYieldSharesToken(agreementId);
        yieldSharesToken = YieldSharesToken(tokenAddress);
        
        // Mint shares to test investors
        yieldSharesToken.mintShares(agreementId, investor1, 30 ether, 30 ether);
        yieldSharesToken.mintShares(agreementId, investor2, 40 ether, 40 ether);
        yieldSharesToken.mintShares(agreementId, investor3, 30 ether, 30 ether);
    ...
    function _querySubgraph(string memory query) internal returns (string memory response) {
        // Build curl command for GraphQL query with separate arguments
        ...
        inputs[5] = "-d";
        inputs[6] = string(abi.encodePacked('{"query":"', query, '"}'));
        inputs[7] = "--silent";
        inputs[8] = GRAPH_NODE_ENDPOINT;
    ...
    {
        // Use vm.parseJson to extract the field value
        // Note: fieldPath should start with "." for vm.parseJson
        try vm.parseJsonUint(response, fieldPath) returns (uint256 parsedValue) {
            return parsedValue;
        } catch {
            // If parsing fails, return 0 and log error
            console.log("Failed to parse JSON field:", fieldPath);
    ...
    function testTotalRepaidAccuracy() public {
        ...
        // Convert agreementId to hex string with 0x prefix and pad to 32 bytes
        string memory agreementIdHex = vm.toString(agreementId);
