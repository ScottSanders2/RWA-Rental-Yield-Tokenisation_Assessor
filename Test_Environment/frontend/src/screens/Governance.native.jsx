...
const Governance = ({ navigation }) => {
  ...
  const fetchProposals = async () => {
    try {
      ...
      console.log(`✅ Received ${data.length} proposals from backend`);
      
      // Parse dates and set proposals
      const proposalsWithDates = data.map(p => ({
        ...p,
        voting_start: new Date(p.voting_start),
        voting_end: new Date(p.voting_end)
      }));
      
      setProposals(proposalsWithDates);
    } catch (err) {
      console.error('❌ Failed to load proposals:', err.message);
      ...
      const mockProposals = [
        {
          ...
          description: 'Increase ROI from 12% to 12.6% (Mock)',
          voting_start: new Date(Date.now() - 24 * 60 * 60 * 1000),
