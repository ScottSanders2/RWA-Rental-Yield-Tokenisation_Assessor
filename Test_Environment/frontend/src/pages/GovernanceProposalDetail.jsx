...
function GovernanceProposalDetail() {
  ...
  const loadVotedStatus = async () => {
    ...
    try {
      // Call backend API to check if user has voted (using current profile's wallet)
      const response = await fetch(
        `http://localhost:8001/governance/proposals/${proposalId}/my-vote?voter_address=${currentProfile.wallet_address}`
      );
      if (response.ok) {
        const data = await response.json();
        if (data.has_voted) {
          setHasVoted(true);
          console.log(`✅ ${currentProfile.display_name} has already voted on proposal ${proposalId}: ${data.support === 0 ? 'Against' : data.support === 1 ? 'For' : 'Abstain'}`);
        } else {
          setHasVoted(false);
        }
      }
    } catch (err) {
      console.error('Error loading voted status from backend:', err);
  ...
  const fetchVotingPower = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8001/users/profiles/${currentProfile.wallet_address}/voting-power/${proposal.agreement_id}`
      );
      if (response.ok) {
        const data = await response.json();
        setVotingPower(data.voting_power);
        console.log(`📊 ${currentProfile.display_name} voting power: ${data.voting_power.toLocaleString()} tokens (${data.percentage}%)`);
      }
    } catch (err) {
  ...
  const fetchProposalDetail = async (preserveVotedStatus = false) => {
    try {
      setLoading(true);
      const response = await fetch(`http://localhost:8001/governance/proposals`);
      if (response.ok) {
        const proposals = await response.json();
        const foundProposal = proposals.find(p => p.proposal_id === parseInt(proposalId));
        if (foundProposal) {
          setProposal(foundProposal);
          // Voting power will be fetched by useEffect when proposal changes
          // Don't set mock data here - let the real API call handle it
        } else {
  ...
  const handleVote = async (support) => {
    ...
    try {
      ...
      // Refresh proposal data while preserving voted status
      await fetchProposalDetail(true);
    } catch (err) {
