...
async def check_user_vote(
    ...
    **Use Case:**
    ...
    try:
        ...
        logger.error(f"Error checking vote status for proposal {proposal_id}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def data_integrity_audit(db: Session = Depends(get_db)):
    ...
    **Use Cases:**
    ...
    try:
        ...
        for proposal in proposals:
            # Check token balances for this agreement
            token_check = db.query(
                func.count(TokenBalance.id).label('holder_count'),
                func.sum(TokenBalance.balance).label('total_tokens')
            ).filter(
                TokenBalance.agreement_id == proposal.agreement_id
            ).first()
