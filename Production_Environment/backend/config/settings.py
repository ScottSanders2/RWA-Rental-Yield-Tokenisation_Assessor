...
class Settings(BaseSettings):
    ...
    postgres_user: str = Field(default="rwa_prod_user", description="PostgreSQL username")
    postgres_password: str = Field(default="prod_password", description="PostgreSQL password")
    postgres_db: str = Field(default="rwa_prod_db", description="PostgreSQL database name")
    postgres_host: str = Field(default="rwa-prod-postgres", description="PostgreSQL host")
    postgres_port: int = Field(default=5432, description="PostgreSQL port")

    # Redis Configuration
    redis_host: str = Field(default="rwa-prod-redis", description="Redis host")
    redis_port: int = Field(default=6379, description="Redis port")
    redis_password: str = Field(default="prod_redis_pass", description="Redis password")

    # Web3/Anvil Configuration
    web3_provider_uri: str = Field(
        default="http://rwa-prod-foundry:8545",
    ...
    cors_origins: List[str] = Field(
        default_factory=lambda: ["http://localhost:3000", "http://localhost:5173", "http://localhost"],
