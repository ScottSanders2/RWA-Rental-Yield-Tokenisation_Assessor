...
"""
Simple approach: 
1. First, reconvert the markdown to Word (clean slate)
2. Then find EVERY Figure placeholder line and insert image + caption properly
"""
    ...
    if match:
        ...
        if fig_num in FIGURES:
            ...
            if os.path.exists(img_path):
                ...
                inserted += 1
                if inserted <= 10 or inserted % 20 == 0:
                    print(f"  ✓ {fig_num}")
            else:
                failed.append(f"{fig_num}: File not found")
        else:
            # Not in our mapping - might be a placeholder we don't have
            pass
